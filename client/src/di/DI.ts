...
export function rangeToSummary(
  ...
  // Currently only handling single-page selections
  ...
  const startWordIndex = findClosestWordIndex(startPage, range.start.point);
  if (startWordIndex == null) {
    console.warn("rangeToSummary | could not find anchor word");
    return {} as Summary;
  }

  // If single-point selection, just return that single word
  ...
  // Find the endpoint word
  const endWordIndex = findClosestWordIndex(endPage, range.end.point);
  if (endWordIndex == null) {
    // fallback: return single word selection
    return createSummary(
      [startPageIdx, startPageIdx],
      [startWordIndex, startWordIndex],
      di
  ...
  if (startPageIdx === endPageIdx) {
    ...
    // TODO: Handle multi-page selection
    return {} as Summary;
  }

  // Build final summary from (page, wordIndex) range
  return createSummary(
...
export function summaryToBounds(
  ...
  summary.polygons.forEach(({ polygon, page }) => {
    let head = polygon.head ? [...polygon.head] : null;
    let body = polygon.body ? [...polygon.body] : null;
    let tail = polygon.tail ? [...polygon.tail] : null;
